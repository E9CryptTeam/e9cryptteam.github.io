...
/**
 * Q4ScheduleBuilder class to manage and display the Q4 pattern schedule.
 */
class Q4ScheduleBuilder {
  ...
  getSchedule(startDate, endDate, cyclePattern) {
    const schedules = [];
    const cycleLength = cyclePattern.length; // Q4 has 9-day cycle
    ...
    while (currentDate <= endDate) {
      ...
      // Calculate which day in the cycle (0 to cycleLength-1)
      // Handle negative dates (before base date) properly
      let dayOfCycle;
      if (dayOffset >= 0) {
        dayOfCycle = dayOffset % cycleLength;
      } else {
        // For dates before base date, wrap properly
        dayOfCycle = ((dayOffset % cycleLength) + cycleLength) % cycleLength;
      }
      ...
      schedules.push({
        ...
        shifts: [cycleDay.shifts[0], cycleDay.shifts[1]],
        off: cycleDay.off,
      });
